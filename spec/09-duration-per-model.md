...
**When:** with video UI + templates. Durations come from **LTX / the video provider**, not a guessed list. the reference product’s 4/5/6/8/10 is a starting set **until** Modal says otherwise.
...
- Only allow a fixed duration set (the reference used 4, 5, 6, 8, 10 — keep that until Modal’s catalog says otherwise). Read `supported_durations` from the **video provider**, do not hardcode per model name.
...
**Rules (locked from the reference product, they are the product)**
...
- Backend: persist `track` + `duration`; enforce `endSec` derived from duration (until crop); compositing must slice the timeline at block edges and pick the topmost clip per slice; gaps = black. Keep overlap resolution in a **pure function** (the reference `buildTimelineSegments`) so it can be unit-tested without GPUs.
