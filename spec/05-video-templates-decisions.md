...
Companion to [`04-video-templates.md`](./04-video-templates.md). the reference 05/06 proved the data model; we keep it and change **where compute runs**.
...
| How does rendering run? | **Async worker**, not the reference blocking request. Same pipeline for admin export and user generate; export also publishes. |
...
## 2. Data model (keep the reference shape)
...
## 3. API (keep the reference paths so Stitch can follow one contract)
...
- Timeline: Premiere-like. reference file names (`Timeline.tsx`, `BlockInspector.tsx`) are hints only.
...
- Email verification.
- Synthesizing a “clean portrait” on avatar create (rejected by the reference product; still rejected — extra cost for no FaceFusion gain).
