...
OpenRouter `/chat/completions` writes to the **chosen film length**: 30, 45, **60 (default)**, or 90 seconds. Scene count and word budget scale with that pick (60s stays 6–8 scenes, 4–8 seconds each, total 45–65s). Generated duration arithmetic is normalized without a second LLM request. Owner edits and coffee presets are validated, not silently normalized; presets are scaled to the chosen length. Full narration word budget tracks spoken length at ~2.5 words/sec (about 65–85 / 100–120 / 140–160 / 210–240 words for 30/45/60/90; generated scripts enforce a floor). Assembly **shrinks picture to the measured TTS duration** when VO is shorter than scene timings; it still holds the last shot when VO runs longer. Measured output and final timeline have a **90s hard cap**. Topic and script fields are prompt-guarded (injection / role-override refused; topic wrapped as data). Project keeps returned text cost when available.
