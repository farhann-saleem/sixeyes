...
5. Parallelise provider jobs where the provider allows (the reference used `Promise.all` with a concurrency cap). Keep a cap so we do not stampede Modal.

Uploaded (non-AI) video blocks stay as uploaded media — do not run them through the avatar/face-swap path (the reference rule; keep it).
