...
import { togglePreview, usePlayingId, WaveBars } from "../audio-ui";
...
export function ProjectWorkspace({ id, step, onStep, onHome, chrome = "full" }: { id: string; step: Step; onStep: (step: Step) => void; onHome: () => void; chrome?: "full" | "none" }) {
  ...
  const [voices, setVoices] = useState<CastVoice[]>([]);
  const [voiceId, setVoiceId] = useState(""); const [voiceError, setVoiceError] = useState("");
  const playingVoiceId = usePlayingId();
    ...
    {step === "script" ? <section className="project-content">
      ...
      {!["cast", "studio", "exported"].includes(project.phase) ? <button onClick={() => onStep("script")}>Review your script first</button> : scenes.map((scene, index) => <article className={`cast-scene ${scene.status === "skipped" ? "skipped" : ""}`} key={scene.id}>
        ...
        <div className="cast-candidates">{scene.candidates.filter((c) => c.kind === "video").map((c, ci) => <div className={`cast-candidate ${c.upload_id === scene.picked_upload_id ? "picked" : ""}`} key={c.upload_id}>
          <div className="cast-preview-frame">
            ...
            <button
              ...
              onClick={() => setInspectItem({
                ...
                download: "",
                title: scene.heading,
                subtitle: DOC_SHOT_MODELS[ci % DOC_SHOT_MODELS.length],
                kind: "video",
              })}
            >
              ⤢
          ...
          <div className="cast-candidate-meta">
            <span className="cast-ai-tag">{DOC_SHOT_MODELS[ci % DOC_SHOT_MODELS.length]}</span>
            <span className="cast-clip-len">{c.duration_s ? `${Math.round(c.duration_s)}s` : "Clip"}</span>
          </div>
        ...
        {!scene.candidates.filter((c) => c.kind === "video").length && <p className="project-empty-small">{scene.status === "skipped" ? "No visual selected. This scene is skipped." : scene.error || "No videos found for this shot prompt. Edit the prompt and retry."}</p>}
      </article>)}
      {!assembled && (() => {
        const selectedVoice = voices.find((v) => v.voice_id === voiceId) || voices[0];
        const previewUrl = selectedVoice?.preview_url || "";
        const voiceLabel = selectedVoice?.name || selectedVoice?.voice_name || selectedVoice?.voice_id || "voice";
        const previewPlaying = Boolean(selectedVoice && playingVoiceId === selectedVoice.voice_id);
        return (
          <footer className="project-actions cast-actions">
            <div className="cast-voice-block">
              <label>
                Narration voice
                <div className="cast-voice-row">
                  <select
                    value={voiceId}
                    disabled={disabled || !!project.tts_job_id}
                    onChange={(e) => setVoiceId(e.target.value)}
                  >
                    {!voices.length ? <option value="">Loading voices…</option> : null}
                    {voices.map((v) => (
                      <option key={v.voice_id} value={v.voice_id}>{v.name || v.voice_name || v.voice_id}</option>
                    ))}
                  </select>
                  <button
                    type="button"
